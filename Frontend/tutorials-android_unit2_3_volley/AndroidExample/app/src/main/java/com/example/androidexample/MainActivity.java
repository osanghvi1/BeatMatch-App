...
public class MainActivity extends AppCompatActivity {
    ...
    String preferredGenres = getUserPreferredGenres();

    private String getUserPreferredGenres() {
        SharedPreferences sharedPreferences = getSharedPreferences("UserPreferences", MODE_PRIVATE);
        StringBuilder preferredGenres = new StringBuilder();

        if (sharedPreferences.getBoolean("pop", false)) {
            preferredGenres.append("Pop, ");
        }
        if (sharedPreferences.getBoolean("rock", false)) {
            preferredGenres.append("Rock, ");
        }
        if (sharedPreferences.getBoolean("hiphop", false)) {
            preferredGenres.append("Hip-Hop, ");
        }

        // Remove trailing comma and space if any genres were added
        if (preferredGenres.length() > 0) {
            preferredGenres.setLength(preferredGenres.length() - 2);
        }

        return preferredGenres.toString();
    }
